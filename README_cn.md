...
- Python：3.10（3.9–3.11 一般也可工作）
- 操作系统：Linux / WSL / Windows（推荐使用带 NVIDIA GPU 的机器，但也可在 CPU 上运行）
- GPU（可选）：与所安装 PyTorch 版本兼容的 CUDA 环境

本仓库同时提供 Conda 环境文件和 pip 依赖文件：

- `environment.yml`：推荐使用的 Conda 环境模板（包含 Python + PyTorch + CUDA + 项目依赖）
- `requirements.txt`：纯 Python 包依赖列表，适合用 `pip` / 虚拟环境安装

### 4.2 主要依赖

完整依赖请参考 `requirements.txt`，这里列出核心部分：
...
- 数据增强与图像处理：
  ...
  - `opencv-python`（服务器/无显示环境可考虑使用 `opencv-python-headless`）
- 可视化与分析：
  - `matplotlib`、`seaborn`
  - `scikit-learn`、`tqdm`
- 其他工具：
  - `PyYAML`、`pydantic`、`typing_extensions` 等

### 4.3 安装方式 A：使用 Conda（推荐）

如果你已安装 Anaconda/Miniconda，推荐使用 `environment.yml` 一键创建环境：

```bash
# 基于 environment.yml 创建并激活 Conda 环境
conda env create -f environment.yml
conda activate forensicvision

# 如有需要，也可以在激活环境后额外安装 pip 依赖：
# pip install -r requirements.txt
```

当前提供的 `environment.yml` 默认假定使用 CUDA 12.1（通过 `pytorch-cuda=12.1` 提供 GPU 加速）。  
如果你的显卡驱动 / CUDA 版本不同，请参考 PyTorch 官网安装说明（https://pytorch.org/），
根据你的实际 CUDA 版本调整环境文件或安装命令。

### 4.4 安装方式 B：使用 Python 虚拟环境 + pip

如果你不想使用 Conda，也可以使用标准的 Python 虚拟环境。  
在这种情况下，你需要自己先安装**兼容的 PyTorch（CPU 或 GPU 版本）**，然后再安装本项目依赖。
...
# 1）请先根据 https://pytorch.org/ 提供的命令安装合适的 PyTorch（CPU 或 GPU）
#    例如（Linux, CUDA 12.1, pip）：
...
> 提示：在 WSL 中运行时，项目路径通常类似 `/home/username/projects/ForensicVision`，
> 与 Windows 的盘符路径不同，请注意区分。
